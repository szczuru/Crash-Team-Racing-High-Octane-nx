...
void howl_InitChannelAttr_Music(struct SongSeq *seq, struct ChannelAttr *attr, int index, int channelVol)
{
	...
	if ((seq->flags & 4) == 0)
	{
		...
		attr->spuStartAddr = (void *)(uintptr_t)(sdata->howl_spuAddrs[longSample->spuIndex].spuAddr << 3);
	...
	else
	{
		...
		// NOTE: spuAddr is a small SPU-RAM offset (not a real CPU pointer);
		// route through uintptr_t to avoid the int->pointer-width warning.
		attr->spuStartAddr = (void *)(uintptr_t)(sdata->howl_spuAddrs[shortSample->spuIndex].spuAddr << 3);
